...
export default function Settings(props) {
  ...
  return (
    <>
      ...
      <br/>
      <br/>
      <div style={{display:filterValue === "custom"?"inline-flex":"none"}}>
        <label style={{ display: "inline-flex",marginRight:"2rem", }} onClick={()=>{setOpen(true)}}>
          <DatePicker
            selected={startDate}
            onChange={(date) => {
        ...
        <Button onClick={()=>{customFilter()}} > Click here to check the unix time</Button>
        <br/>
        <br/>
      </div>
      <div >
